...
public class OSI {
    private static void inputStreamCopy(String path, String fileInName, String fileOutName) {
        ...
        try {
            ...
            int c;
            String line = "";
            osiReverse.add(0, "\n"); //промежуток между двумя записями
            while ((c = in.read()) != -1) {
                if ((char) c != '\n') {
                    line += (char) c;
                } else {
            ...
            System.out.println("Osi reversed in " + fileOutName + " with the help of FileInputStream-FileOutputStream.");
        }
        catch (IOException e) {
    ...
    private static void bufferedReaderCopy(String path, String fileInName, String fileOutName) {
        try (BufferedReader reader = Files.newBufferedReader(Paths.get(path + fileInName), Charset.defaultCharset())) {
            ...
            try (BufferedWriter writer = Files.newBufferedWriter(Paths.get(path + fileOutName), Charset.defaultCharset(), APPEND)) {
                ...
                System.out.println("Osi reversed in " + fileOutName + " with the help of BufferedReader.");
            }
            catch (IOException e) {
    ...
    public static void main(String[] args) {
        ...
        inputStreamCopy(path, "osi.txt", "osiReverse.txt");
        bufferedReaderCopy(path, "osi.txt", "osiReverse.txt");
    }
}
